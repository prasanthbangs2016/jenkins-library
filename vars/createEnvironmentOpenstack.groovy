...
""")
            ...
            withCredentials([file(credentialsId: 'prvcld-openrc-caasp-ci-tests', variable: 'OPENRC')]) {
                ...
                if (image == null || image == '') {
                    ...
                    image = sh(script: "set -o pipefail; set +x; source $OPENRC; openstack image list --property caasp-channel='devel' --property caasp-version='2.0.0' -c Name -f value | sort -r -V | head -n1 | tr -d \"\n\"", returnStdout: true)
                }
