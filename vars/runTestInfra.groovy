...
def call(Map parameters = [:]) {
    ...
    dir("automation/testinfra") {
        withEnv([
            ...
            // We'll run 1 parallel instance of testinfra for each node, which will run on the
            // jenkins worker. These are light processes, as they simply SSH to the target node
            // and run the bulk of tests there. Each test is limited to a single node by the very
            // nature of testinfra, so running in parallel is harmless.
            def parallelSteps = [:]
